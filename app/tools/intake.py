...
@tool(
    ...
    description=(
        ...
        "Every phase's checks can collapse into one call this way — see "
        "spec 4.5. Below is every phase with a documented batch list. "
        "Phases whose checks read the original upload use attachment_id; "
        "phases whose checks read content the model is actively composing "
        "(a drafted section, a cover letter, a JD) have no attachment_id "
        "shortcut — supply that content directly in the shared inputs dict "
        "instead, but still send every tool_id from that phase in one call. "
        "For Phase 0 (Starting Point): tool_ids=[T-0.2,T-0.6,T-0.7,T-0.8,"
        "T-0.9], inputs={ocr_confidence, replacement_char_ratio, "
        "role_blocks_with_dates, date_parse_failure_ratio (T-0.2's own "
        "signals), sections (T-0.6, the parsed career-inventory dict), "
        "fields, required_fields (T-0.7), items, threshold (T-0.8, "
        "threshold optional), claim_text, source_text (T-0.9)}. "
        ...
        "section; each tool reads only the key(s) it declares. "
        "For Phase 4 (Formatting), whole-document checks: "
        "tool_ids=[T-4.1,T-4.2,T-4.3,T-4.4,T-4.9], inputs={attachment_id: "
        "<id>}. T-4.5, T-4.6, T-4.7, T-4.14 are NOT docx-based — each "
        "reads one snippet directly (date_range_text [+ is_current_role, "
        "reference_date] for T-4.5/T-4.6, header_text for T-4.7, "
        "contact_text for T-4.14), so they can only check one instance "
        "per call. Still batch them together with each other and the "
        "attachment_id group in the same call when checking a single "
        "instance of each; if the resume has multiple roles or section "
        "headers, call run_batch_checks again per additional instance, "
        "same shape as T-3.17. "
        "For Phase 5 (Fit Check): tool_ids=[T-6.6,T-6.7,T-6.8,T-6.15], "
        "inputs={bullets, headline_text, posting_title, jd_phrases, "
        "resume_text} (T-6.15 only needs session, already threaded through — "
        "no extra input required for it). "
        "For Phase 7 (Cover Letter): tool_ids=[T-7.1,T-7.2,T-7.3,T-7.4,"
        "T-7.5,T-7.12], inputs={letter_text, salutation, cover_letter_font, "
        "resume_font, jd_text}. "
        "For Phase 8 (Final Review): tool_ids=[T-8.2,T-8.5,T-8.6,T-8.7,"
        "T-8.8,T-8.9,T-8.12,T-8.13,T-8.14,T-8.15], "
        "inputs={attachment_id: <id>, fact_id, claimed_value (T-8.2 — one "
        ...
        "spec — call it after these programmatic results come back, not "
        "in the same batch. "
