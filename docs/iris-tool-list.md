...
| T-2.17 | check_facts_traceable_to_master | TOOL | Fallback path when no profile file exists. First session, or a user who lost theirs. |
...
| T-2.19 | restore_registry_from_profile | TOOL | Added 2026-07-27. Writes a validated profile's Locked Facts Registry back onto the session. Deterministic deserialization, so TOOL. Exists because export/validate/apply was only two-thirds built: T-2.14 serialized the registry and T-2.15 validated it, but nothing wrote it back, so a returning user restored dismissed findings and nothing else and was then blocked at Fit Check by T-5.2. Refuses to overwrite a session that already has active facts. Restoring is not verifying: T-2.17 remains the check that a rehydrated registry still traces to a real master. |
...
| T-8.3 | nominate_added_clauses | HYBRID | Diff tailored against master, isolate added spans, check each against the registry with full rigor. The diff is code; the check is judgment. Failure pattern (a). |
...
| T-8.9 | check_figures_against_master | TOOL | Extraction plus comparison. |
